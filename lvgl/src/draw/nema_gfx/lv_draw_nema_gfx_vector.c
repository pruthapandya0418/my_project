...
static void task_draw_cb(void * ctx, const lv_vector_path_t * path, const lv_vector_path_ctx_t * dsc)
{
	lv_vector_path_t * modifiable_path = (lv_vector_path_t *)path;
    ctx_t * c = ctx;
    ...
    uint8_t * nema_ops;
    if(modifiable_path->nema_ops_cache && modifiable_path->nema_ops_cache_size == op_count) {
        nema_ops = modifiable_path->nema_ops_cache;
    } else {
        nema_ops = lv_malloc(op_count * sizeof(*nema_ops));
        ...
        if(modifiable_path->nema_ops_cache) lv_free(modifiable_path->nema_ops_cache);
        modifiable_path->nema_ops_cache = nema_ops;
        modifiable_path->nema_ops_cache_size = op_count;
    }
